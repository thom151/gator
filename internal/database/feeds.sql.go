...
const getFeedCreator = `-- name: GetFeedCreator :many
SELECT users.name, feeds.name, feeds.url 
FROM users INNER JOIN feeds on users.id = feeds.user_id
`

type GetFeedCreatorRow struct {
	Name   string
	Name_2 string
	Url    string
}

func (q *Queries) GetFeedCreator(ctx context.Context) ([]GetFeedCreatorRow, error) {
	rows, err := q.db.QueryContext(ctx, getFeedCreator)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetFeedCreatorRow
	for rows.Next() {
		var i GetFeedCreatorRow
